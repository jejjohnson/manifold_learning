...
gtVec = double(reshape(imgGT, [numRows*numCols 1]));
...
switch dataset
    ...
    otherwise
        error('unrecognized eigenvalues chosen for dataset');
end
...
save_str = char([ save_path sprintf('sep_%s_alpha_%s_k%d', ...
    dataset, classOptions.method, 20)]);
...
save_str = char([ save_path sprintf('se_%s_alpha_%s_k%d', ...
    dataset, classOptions.method, 20)]);
